...
counter = counter+1; 
load(['./tomoData_other/tomo_filmCase_' num2str(filmCase) '_filterCase_' num2str(filterCase) '_polychromatic_withFilm.mat'],'tomoNoisy');
tomoWithFilm = tomoNoisy;
load(['./tomoData_other/tomo_filmCase_' num2str(filmCase) '_filterCase_' num2str(filterCase) '_polychromatic_empty.mat'],   'tomoNoisy');
tomoEmpty = tomoNoisy;
tomoDiff = tomoWithFilm - tomoEmpty ;
save(['./tomoData_other/tomo_filmCase_' num2str(filmCase) '_filterCase_' num2str(filterCase) '_polychromatic_diff.mat'],'tomoDiff')
...
for i=1:5
    ...
    for j=1:8
        plot(linspace(0,90,length(integralDataAllRaw{i,j})),integralDataAllRaw{i,j})
    end
