...
totalMeasuringTime = 6; % total measuring time over all projections in seconds
...
for filmCase = 1%:5
    ...
    for Ebin = energyBinList
        ...
        %#/keV/pixel/projection @ 1 mete
        % Y_nps= energy-binned version of number of flat field photons in
        % the per pixel and exposure time
    ...
    %
    save(['./tomoData_other/tomo_filmCase_' num2str(filmCase) '_filterCase_' num2str(filterCase) '_' spectrumType '_' imgName '.mat'],'tomoNoisy')
    %
...
B03_visualizeResults_forVariations
toc