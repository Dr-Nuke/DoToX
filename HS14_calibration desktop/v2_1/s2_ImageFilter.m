...
for i =3%1:size(flag) %iterate cases
    ...
    parfor j=20%1:jmax
        ...
        if xls{i}{j,7}~=0
            ...
            if ind_write==1
                f2_parforSave(strcat(wpath,wfile),im)  % parfor-proof save func
            end
...
% save
if ind_write==1
    f2_parforSave(strcat(wpath,wfile),im)
end

%% OB

imax=5;
rpath=strcat(writepath,'1raw\ob_\');
wpath=strcat(writepath,'2flt\ob_\');
disp('filtering OB')

parfor i =1:imax,
    rfile=strcat('ob_',num2str(i),'.fits');
    wfile=strcat('ob_filt',num2str(i),'.mat');
            im=im2double(fitsread(strcat(rpath,rfile))');
