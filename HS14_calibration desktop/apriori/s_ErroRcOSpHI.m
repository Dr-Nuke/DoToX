...
for ii=1:length(varlist)
    dx=(xtot{ii}(:)-phanctot{ii}(:))./phanctot{ii}(:);    
        ...
        p(ii)=plot(dx,'DisplayName','|deviation|',...
            'Color',f4_Colorgen(jet,1,length(varlist),ii),...
            'DisplayName',sprintf('%4.3f',varlist(ii)));
    ...
    title('The influence of the cutoff of cos(phi) on the standard deviation')
    %ylim([-0.2,0.2])

end
% %%
legend([p(1),p(5),p(10),p(15),p(20)],...
    sprintf('%4.3f',varlist(1)),...
    sprintf('%4.3f',varlist(5)),...
    sprintf('%4.3f',varlist(10)),...
    sprintf('%4.3f',varlist(15)),...
    sprintf('%4.3f',varlist(20)))
