...
%% s3_LFT
    ...
    F.r_min=40;               % empirical check shows
    F.S=0.99;                  %sensitivity threshold
    F.o =10;                  % overlap

    % seems unused: F.n_path=round((90/360)*2*pi*F.r_rod*res);  % number of sample paths per pin 
    F.r_path=round(6.7*res)-5; % path length in pixel
   
    F.n_pins = 4; % number of fuel pins
    F.d_angle=180; % angel range
    F.n_angles=F.d_angle+1; % number of angles
    F.h=size(block,3); % stack height
    ...
    centeringrange=350:1019;
    
    % parfor- workaround
    
    
    disp('finding pin centers...')
    parfor k=centeringrange%imrange(2,2) %iterate planes
        f.f_BoCount(k,20,10,5)
        %slice data
        ...
        imbw=im2bw(f_normalize(im),0.4); %empirically chosen threshold of 0.6 for greylevel to bw coonversion
        
        % find centers & radii
        try
            [c(k,:,:),rad(k,:),m(k,:)]=f_hugh_4_2(imbw,F.r_min,F.dr,F.S,F.o,0.4); %have this file in the same folder
            if ~all(rad(k,:))
        ...
        f.f_BoCount(i,20,10,5)
         for j=1:n_pins
            for k=1:n_angles
                ...
                ImProfiles(i,j,k,:)=...
                reshape(improfile(block2(:,:,i)',... % profile of the image
...
figure(28)
clf
imshow(squeeze(block(:,:,i)'),[0,0.001]);set(gca,'YDir','normal') % image
hold on
scatter(F.c(i,:,1),F.c(i,:,2),'rx')
plot(F.c(i,:,1),F.c(i,:,2),'b')
plot(F.cfit(i,:,1),F.cfit(i,:,2),'g')
for j=1:4 %rod
    for k=1:10:F.n_angles %angle
        plot([F.cfit(i,j,1),F.ProfEndPnt(i,j,k,1)],...
             [F.cfit(i,j,2),F.ProfEndPnt(i,j,k,2)]) %plot beams
    end
end
title(i)
%waitforbuttonpress;
