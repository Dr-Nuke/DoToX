...
if  any(itr_stp==7) % execute this script?
    ...
    recsize=999;
    recsize=ceil(recsize/refac);
    ind_nan=zeros(1,imrange(2,2));
    recontot=zeros(recsize,recsize,imrange(2,2),'single');
...
%%
    ...
    for l=1:ceil(imrange(2,2)/parforBin)
        a=(l-1)*parforBin+1;
        b=l*parforBin;
        disp([l,a,b])
        %parfor k=a:b
        ...
        if k <= imrange(2,2)    
            ...
            % cropping
            sino=sino(sinocrop+1:end-sinocrop,:);
            disp([k,size(sino)])
            try
                
               %recon=iradon(circshift(sino(:,1:end-1),11,2),angles,'spline','Hann',1,999);
                recon=iradon(circshift(sino(:,1:end-1),11,2),angles,'spline','Hann',1,recsize);
                %recontot(:,:,k)=recon(100:850,100:850); %hardcode after-recon-cropping
                recontot(:,:,k)=recon; %hardcode after-recon-cropping
            catch
